 ...
 */
function renderVariableValue(thing: any): string {
 ...
 */
export class ToHtmlStringVisitor {
    options: any;
    ...
    static visitChildren(visitor: ToHtmlStringVisitor, thing: any, parameters?: any): string {
        if (!parameters) {
            parameters = { result: '', first: false, indent: 0 };
        }
        if (thing.nodes) {
            thing.nodes.forEach((node: any) => {
    ...
    visit(thing: any, parameters: any): void {
        switch (thing.getType()) {
            case 'Clause': {
                ...
                if (thing.elementType) { attributes += ` elementType="${thing.elementType}"`; }
                if (thing.src) { attributes += ` src="${thing.src}"`; }
            ...
            case 'Variable': {
                ...
                if (thing.elementType) { attributes += ` elementType="${thing.elementType}"`; }
                if (thing.identifiedBy) { attributes += ` identifiedBy="${thing.identifiedBy}"`; }
            ...
            case 'Formula': {
                ...
                if (thing.code) { attributes += ` code="${encodeURIComponent(thing.code)}"`; }
                if (thing.dependencies) { attributes += ` dependencies="${encodeURIComponent(JSON.stringify(thing.dependencies))}"`; }
...
export default ToHtmlStringVisitor;
