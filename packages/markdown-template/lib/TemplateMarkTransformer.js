...
var _throwParseException = require('./errorutil')._throwParseException;
var {
  templateMarkManager,
  templateToTokens,
...
var normalizeToMarkdownCicero = require('./normalize').normalizeToMarkdownCicero;
var normalizeFromMarkdownCicero = require('./normalize').normalizeFromMarkdownCicero;
...
class TemplateMarkTransformer {
   ...
   */
  constructor() {
    var parsingTable = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  ...
  dataFromCiceroMark(input, parserManager, options) {
    var serializer = parserManager.getSerializer();
    var parser = parserManager.getParser();

    // Load the markdown input
    var markdown = normalizeToMarkdownCicero(input.content);
    var markdownFileName = input.fileName;
    ...
    if (result.status) {
      var utcOffset = parserManager.getUtcOffset();
      var output = serializer.toJSON(serializer.fromJSON(result.value, {
        utcOffset
      }), {
        utcOffset
      });
  ...
  fromCiceroMark(input, templateMark, modelManager, templateKind, currentTime, utcOffset, options) {
    ...
    var parserManager = new ParserManager(modelManager, this.parsingTable, templateKind, options);
  ...
  fromMarkdownCicero(markdownInput, templateInput, modelManager, templateKind, currentTime, utcOffset, options) {
    ...
    var typedTemplate = this.fromMarkdownTemplate(templateInput, modelManager, templateKind, options);

    // Load the markdown input
    var ciceroMark = {
      fileName: markdownInput.fileName,
      content: normalizeFromMarkdownCicero(markdownInput.content)
    };
  ...
  draftCiceroMark(data, parserManager, templateKind, options) {
    ...
    var input = templateMarkManager.serializer.fromJSON(parserManager.getTemplateMark());
  ...
  instantiateCiceroMark(data, templateMark, modelManager, templateKind, currentTime, utcOffset, options) {
    // Construct the template parser
    var parserManager = new ParserManager(modelManager, this.parsingTable, templateKind);
    parserManager.setCurrentTime(currentTime, utcOffset);
    parserManager.setTemplateMark(templateMark);
